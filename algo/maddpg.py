...
from network.actor import OpenaiActor as net_a
from network.critic import OpenaiCritic as net_c
...
class MADDPG(BaseAlgo):
    def __init__(self, args):
        ...
        for n in range(num_agent):
            ...
            in_c = [
                sum(obs_dim),
                sum(action_dim),
            ]  # critic的输入是act(cat(obs), cat(action))
    ...
    def learn(self):
        self.step += 1
        if (
            self.step < self.args.start_step
            or not self.step % self.args.target_replace == 0
        ):
            return
        for id in range(self.n):
            ...
            for agent in self.agents:
                ...
                reward.append(
                    ...
                    torch.tensor(
                        [
                            torch.tensor(done, dtype=torch.float64)
            ...
            y_target = reward + self.gamma * torch.mul(
                (1 - done), agent.critic_target(obs_, action_).squeeze().detach()
            )
            y_eval = agent.critic_eval(
                torch.cat(obs, dim=1), torch.cat(action, dim=1)
            ).squeeze()
        ...
        for id in range(self.n):
            agent = self.agents[id]
            for p_target, p_eval in zip(
                agent.actor_target.parameters(), agent.actor_eval.parameters()
            ):
                p_target.data.copy_(
                    (1 - self.tao) * p_target.data + self.tao * p_eval.data
                )
            for p_target, p_eval in zip(
                agent.critic_target.parameters(), agent.critic_eval.parameters()
            ):
                p_target.data.copy_(
                    (1 - self.tao) * p_target.data + self.tao * p_eval.data
                )
