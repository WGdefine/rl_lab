...
from utils.buffer import Replay_buffer as buffer
...
class SAC(BaseAlgo):
    def __init__(self, args) -> None:
        ...
        self.gamma = self.algo_param.gamma
        self.tau = self.algo_param.tau

        self.action_continuous = args.action_continuous

        self.batch_size = self.algo_param.batch_size
        self.hidden_size = self.algo_param.hidden_size
        self.actor_lr = self.algo_param.a_lr
        self.critic_lr = self.algo_param.c_lr
        self.alpha_lr = self.algo_param.alpha_lr

        self.buffer_size = self.algo_param.buffer_capacity
        ...
        self.preset_alpha = self.algo_param.alpha

        if self.policy_type == "deterministic":
            self.tune_entropy = False
            hid_layer = self.algo_param.num_hid_layer
            ...
            self.tune_entropy = self.algo_param.tune_entropy
            self.target_entropy_ratio = self.algo_param.target_entropy_ratio
        ...
        self.eps = self.algo_param.epsilon
        self.eps_end = self.algo_param.epsilon_end
        self.eps_delay = 1 / (self.algo_param.max_episodes * 100)

        self.learn_step_counter = 0
        self.target_replace_iter = self.algo_param.target_replace
        ...
        if self.tune_entropy:
            ...
            self.alpha_optim = Adam([self.log_alpha], lr=self.alpha_lr).to(self.device)
        else:
    ...
    def choose_action(self, observation):
        ...
        if self.policy_type == "discrete":
            action, _, _, _ = self.policy.sample(observation)
            action = action.item()
            ...
            _, _, _, action = self.policy.sample(observation)
            action = action.item()
            self.add_experience({"action": action})
    ...
    def save(self, episode):
        save_path = "models/config_training"
